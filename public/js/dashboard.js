...
async function getContent(){
  const html = await fetch('http://localhost:82/restaurant_test/expenses/create').then(res => res.text());
  return html;
}
...
google.charts.load('current', {'packages':['bar']});
      ...
      async function drawChart() {
        const http = await fetch('http://localhost:82/restaurant_test/expenses/getExpensesJSON')
        .then(json => json.json())
        .then(res => res);
