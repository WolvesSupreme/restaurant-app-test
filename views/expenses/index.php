...
<link rel="stylesheet" href="<?php echo constant('URL') ?>public/css/history.css">
    ...
    <script>
        ...
        async function getData(){
            data = await fetch('http://localhost:8080/restaurant_test/expenses/getHistoryJSON')
            .then(res =>res.json())
            .then(json => json);
            this.copydata = [...this.data];
            console.table(data);
        ...
        function renderData(data){
            ...
            data.forEach(item => { 
                ...
                databody.innerHTML += `<tr>
                        ...
                        <td>${item.date}</td>
                        <td>$${item.amount}</td>
                        <td><a href="http://localhost:8080/restaurant_test/expenses/delete/${item.id}">Eliminar</a></td>
                    </tr>`;
            });
        }
